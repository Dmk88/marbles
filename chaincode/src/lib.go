...
func payment_done_for_offer(offer *Offer, accountId, stellar_transaction_id string) (bool, error) {
	...
	paymentAmount, err := strconv.Atoi(payment.Amount)
	if err != nil {
		return false, errors.New("Unable to parse amount in payment")
	}

	if payment.To == accountId && paymentAmount == offer.OfferPrice && payment.Memo.Type == "Offer" && payment.Memo.Value == offer.Id {
		return true, nil
	} else {
		return false, nil
	}
